...
abstract class AbstractDaemonToolchainInvalidCriteriaIntegrationTest extends AbstractDaemonToolchainIntegrationSpec {
    ...
    def "Given non-integer toolchain version When execute any task Then fails with expected exception message"() {
        ...
        expect:
        failsSimpleTaskWithDescription("Value 'stringVersion' given for daemon.jvm.toolchain.version Build property is invalid (the value should be an int)")
    }
    ...
    def "Given negative toolchain version When execute any task Then fails with expected exception message"() {
        ...
        expect:
        failsSimpleTaskWithDescription("Value '-1' given for daemon.jvm.toolchain.version Build property is invalid (the value should be a positive int)")
    }
    ...
    def "Given undefined toolchain version but valid vendor and implementation When execute any task Then fails with expected exception message"() {
        ...
        expect:
        failsSimpleTaskWithDescription("Option daemon.jvm.toolchain.version undefined on Build properties. Execute 'updateDaemonJvm' task with desired criteria to fix it.")
    }
    ...
    def "Given unexpected toolchain vendor When execute any task Then fails with expected exception message"() {
        ...
        expect:
        failsSimpleTaskWithDescription("Option daemon.jvm.toolchain.vendor doesn't accept value 'unexpectedVendor'. Possible values are " +
    ...
    def "Given unexpected toolchain implementation When execute any task Then fails with expected exception message"() {
        ...
        expect:
        failsSimpleTaskWithDescription("Option daemon.jvm.toolchain.implementation doesn't accept value 'unknownImplementation'. Possible values are [vendor-specific, J9]")
    }
}
