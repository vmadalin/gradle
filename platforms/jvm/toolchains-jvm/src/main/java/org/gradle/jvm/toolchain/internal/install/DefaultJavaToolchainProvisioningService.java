...
public class DefaultJavaToolchainProvisioningService implements JavaToolchainProvisioningService {
    ...
    public File tryInstall(JavaToolchainSpec spec) {
        if (!isAutoDownloadEnabled()) {
            throw new ToolchainProvisioningNotConfiguredException(spec, buildPlatform, "Toolchain auto-provisioning is not enabled.",
        ...
        if (repositories.isEmpty()) {
            throw new ToolchainProvisioningNotConfiguredException(spec, buildPlatform, "Toolchain download repositories have not been configured.",
                "Learn more about toolchain auto-detection at " + Documentation.userManual("toolchains", "sec:auto_detection").getUrl() + ".",
        ...
        if (successfulProvisioning == null) {
            throw downloadFailureTracker.buildFailureException(spec, buildPlatform);
        } else {
    ...
    private static class ToolchainDownloadFailureTracker {
        ...
        public ToolchainProvisioningNotConfiguredException buildFailureException(JavaToolchainSpec spec, BuildPlatform buildPlatform) {
            ...
            ToolchainProvisioningNotConfiguredException exception = new ToolchainProvisioningNotConfiguredException(spec, buildPlatform, cause, resolutions);
