...
class JavaToolchainDownloadSpiIntegrationTest extends AbstractIntegrationSpec implements JavaToolchainFixture {
    ...
    def "can inject custom toolchain registry via settings plugin"() {
        ...
        failure.assertHasDescription("Could not determine the dependencies of task ':compileJava'.")
               ...
               .assertHasCause("Cannot find a Java installation on your machine matching toolchain requirements: {languageVersion=99, vendor=matching('exotic'), implementation=vendor-specific} " +
                   ...
                   "({languageVersion=99, vendor=matching('exotic'), implementation=vendor-specific}) from 'https://exoticJavaToolchain.com/java-99', due to: " +
    ...
    def "downloaded JDK is checked against the spec"() {
        ...
        failure.assertHasDescription("Could not determine the dependencies of task ':compileJava'.")
               .assertHasCause("Failed to calculate the value of task ':compileJava' property 'javaCompiler'.")
               .assertHasCause("Cannot find a Java installation on your machine matching toolchain requirements: {languageVersion=11, vendor=any, implementation=vendor-specific} " +
                   "for ${expectedBuildPlatformFailureMessage()} and some toolchain resolvers had provisioning failures: custom (Unable to download toolchain matching the requirements " +
                   "({languageVersion=11, vendor=any, implementation=vendor-specific}) from '$uri', due to: Toolchain provisioned from '$uri' doesn't satisfy the specification: " +
    ...
    def "throws informative error on repositories not being configured"() {
        ...
        failure.assertHasDescription("Could not determine the dependencies of task ':compileJava'.")
               .assertHasCause("Failed to calculate the value of task ':compileJava' property 'javaCompiler'.")
               .assertHasCause("Cannot find a Java installation on your machine matching toolchain requirements: {languageVersion=99, vendor=any, implementation=vendor-specific} " +
