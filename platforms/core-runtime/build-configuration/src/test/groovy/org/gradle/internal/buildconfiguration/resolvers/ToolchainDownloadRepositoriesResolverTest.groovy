...
class ToolchainDownloadRepositoriesResolverTest extends Specification {
    ...
    def "Given no configured repositories When resolve toolchain spec download urls Then expected exception is thrown"() {
        ...
        def resolver = new ToolchainDownloadRepositoriesResolver(registry, TestUtil.objectFactory())

        when:
        resolver.resolveToolchainDownloadUrlsByPlatform(11, JvmVendor.KnownJvmVendor.AMAZON.asJvmVendor(), JvmImplementation.VENDOR_SPECIFIC)
    ...
    def "Given single configured repositories When unable to resolve toolchain spec download urls Then emtpy urls are returned by supported platforms"() {
        ...
        when:
        def result = resolver.resolveToolchainDownloadUrlsByPlatform(11, JvmVendor.KnownJvmVendor.AMAZON.asJvmVendor(), JvmImplementation.VENDOR_SPECIFIC)
    ...
    def "Given single configured repositories When resolve toolchain spec download urls Then expected urls are returned by supported platforms"() {
        ...
        when:
        def result = resolver.resolveToolchainDownloadUrlsByPlatform(17, JvmVendor.KnownJvmVendor.AZUL.asJvmVendor(), JvmImplementation.VENDOR_SPECIFIC)
    ...
    def "Given multiple configured repositories When resolve toolchain spec download urls Then first resolved valid urls are returned by supported platforms"() {
        ...
        when:
        def result = resolver.resolveToolchainDownloadUrlsByPlatform(8, JvmVendor.KnownJvmVendor.IBM.asJvmVendor(), JvmImplementation.J9)
    ...
    private List<BuildPlatform> expectedToolchainSupportedPlatforms() {
        return [Architecture.AARCH64, Architecture.X86_64].stream()
            .flatMap(architecture -> OperatingSystem.values().toList().stream().map(operatingSystem -> new DefaultBuildPlatform(architecture, operatingSystem)))
