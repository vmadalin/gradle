...
//        registration.add(MavenToolchainsInstallationSupplier.class);
    ...
    protected BuildPlatform createBuildPlatform() {
        return new DefaultBuildPlatform(Architecture.current(), OperatingSystem.current());
    }
    ...
    @Provides
    protected JdkCacheDirectory createJdkCacheDirectory(GradleUserHomeDirProvider gradleUserHomeDirProvider, FileOperations fileOperations, FileLockManager fileLockManager, JvmMetadataDetector jvmMetadataDetector, GradleUserHomeTemporaryFileProvider gradleUserHomeTemporaryFileProvider) {
        return new DefaultJdkCacheDirectory(gradleUserHomeDirProvider, fileOperations, fileLockManager, jvmMetadataDetector, gradleUserHomeTemporaryFileProvider);
    }
